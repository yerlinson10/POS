            ...
            <div
                ...
                <div class="flex flex-col gap-2 md:gap-4 p-2 md:p-6 border-b bg-muted/30">
                    <div class="flex flex-col xs:flex-row xs:items-center justify-between gap-2 xs:gap-0">
                        <div>
                            <h2 class="text-base md:text-xl font-semibold">Categories Management</h2>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-2 text-xs md:text-sm text-muted-foreground">
                                <Icon name="FolderOpen" class="w-3 h-3 md:w-4 md:h-4" />
                                <span v-if="categories.total">{{ categories.total }} items</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="relative flex-1">
                            ...
                            <Input v-model="filters.search" placeholder="Search categories..."
                    ...
                    <div class="h-full overflow-y-auto">
                        <table class="w-full">
                            <thead
                                ...
                                <tr class="h-12">
                                    <th class="text-left px-4 py-3 font-semibold text-sm">ID</th>
                                    <th class="text-left px-4 py-3 font-semibold text-sm">Category Name</th>
                                    <th class="text-center px-4 py-3 font-semibold text-sm w-28">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="p in categories.data" :key="p.id"
                ...
                <div v-if="categories.data.length > 0"
                    ...
                    <Pagination v-slot="{ page: internalPage }" :items-per-page="filters.per_page"
                        ...
                        <PaginationContent v-slot="{ items: pages }" class="justify-center sm:justify-end">
                            ...
                            <PaginationEllipsis :index="4" v-if="categories.last_page >= 4" class="h-8 md:h-9" />
                            <PaginationNext @click="onPageChange(internalPage + 1)"
                                :disabled="categories.current_page >= categories.last_page" class="h-8 md:h-9" />
                        </PaginationContent>
                    </Pagination>
                </div>
            </div>
...
const props = defineProps<{
    categories: import('@/composables/useCategoryFilters').Paginated<import('@/composables/useCategoryFilters').Category>
    filters: import('@/composables/useCategoryFilters').CategoryFilters
}>()
