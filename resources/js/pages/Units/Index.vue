            ...
            <div
                ...
                <div class="flex flex-col gap-2 md:gap-4 p-2 md:p-6 border-b bg-muted/30">
                    <div class="flex flex-col xs:flex-row xs:items-center justify-between gap-2 xs:gap-0">
                        <div>
                            <h2 class="text-base md:text-xl font-semibold">Units Measurement</h2>
                            <p class="text-xs md:text-sm text-muted-foreground hidden xs:block">
                                Manage your measurement units for products
                            </p>
                        </div>
                        <div class="flex items-center gap-2">
                            <Button as="a" href="/unit-measures/create" size="sm" class="h-8 md:h-11 px-2 md:px-6 text-xs md:text-sm">
                            ...
                            <div class="flex items-center gap-2 text-xs md:text-sm text-muted-foreground">
                                <Icon name="Ruler" class="w-3 h-3 md:w-4 md:h-4" />
                                <span v-if="units.total">{{ units.total }} items</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="h-full overflow-y-auto">
                        <table class="w-full">
                            <thead
                                ...
                                <tr class="h-12">
                                    <th class="text-left px-4 py-3 font-semibold text-sm">Code</th>
                                    <th class="text-left px-4 py-3 font-semibold text-sm">Name</th>
                                    <th class="text-center px-4 py-3 font-semibold text-sm w-28">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="p in units.data" :key="p.id"
                                    class="border-b hover:bg-muted/50 transition-colors group">
                                    <!-- Code -->
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-2">
                                            <span class="font-mono bg-primary/10 text-primary px-2 py-1 rounded-md text-sm font-medium">{{ p.code }}</span>
                                        </div>
                                    </td>
                ...
                <!-- Compact Pagination -->
                <div v-if="units.data.length > 0" class="flex flex-col xs:flex-row xs:items-center justify-between gap-2 xs:gap-4 p-2 md:p-4 border-t bg-muted/20">
                    <div class="flex flex-col xs:flex-row xs:items-center gap-1 xs:gap-4">
                        <div class="text-xs text-muted-foreground">
                            <span class="font-medium">{{ units.from || 0 }}-{{ units.to || 0 }}</span>
                            <span class="hidden xs:inline"> of </span>
                            <span class="xs:hidden">/</span>
                            <span class="font-medium">{{ units.total }}</span>
                        </div>
                        <div class="text-xs text-muted-foreground">
                            Page {{ units.current_page }}/{{ units.last_page }}
                        </div>
                    </div>

                    <Pagination v-slot="{ page: internalPage }" :items-per-page="filters.per_page"
                        :total="units.last_page" :page="filters.page" @page-change="onPageChange"
                        class="flex">
                        <PaginationContent v-slot="{ items: pages }" class="justify-center sm:justify-end">
                            <PaginationPrevious @click="onPageChange(internalPage - 1)"
                                :disabled="units.current_page <= 1" class="h-8 md:h-9" />
                            ...
                            <PaginationEllipsis :index="4" v-if="units.last_page >= 4" class="h-8 md:h-9" />
                            <PaginationNext @click="onPageChange(internalPage + 1)"
                                :disabled="units.current_page >= units.last_page" class="h-8 md:h-9" />
                        </PaginationContent>
                    </Pagination>
                </div>
            </div>
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Units Measurement', href: '/unit-measures' },
]

const props = defineProps<{
    units: import('@/composables/useUnitFilters').Paginated<import('@/composables/useUnitFilters').Unit>
    filters: import('@/composables/useUnitFilters').UnitFilters
}>()
